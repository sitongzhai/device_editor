...
#define FREE(ptr) if(ptr){free((char*) ptr); ptr = 0;}
  ...
  char* EDACommandsGenerator(const char* text, int state) {
  ...
  char* EDACommandsOptionGenerator(const char* text, int state) {
    static int list_index;
    static size_t len;
    if (text == NULL || text[0] != '-')
      return ((char*)NULL);
  ...
  char* EDATclVarGenerator(const char* text, int state) {
    static int list_index;
    static size_t len;
    static Tcl_Obj** objv;
    static int objc = 0;
  ...
  char** EDACommandsCompletion(const char* text, int start, int end) {
    char** matches;
    matches = (char**)NULL;
    if (start == 0)
      matches = rl_completion_matches(text, EDACommandsGenerator);
    else if (text && text[0] == '-')
      matches = rl_completion_matches(text, EDACommandsOptionGenerator);
    else if (text && text[0] == '$')
      matches = rl_completion_matches(text, EDATclVarGenerator);
    return matches;
  }
  void EDAReadLineInit(const char* history_file) {
    rl_readline_name = "eda_tclreadline";
    ...
    rl_attempted_completion_function = EDACommandsCompletion;
  ...
  void EDAReadLineLoop(Tcl_Interp* interp) {
#ifndef WIN32
    const char* prompt = prompt_prefix.c_str();
    tclrl_interp = interp;
    
    EDAReadLineInit(default_history_file.c_str());
#endif
    if (init_tcl_file) {
    ...
    while (true) {
      ...
      if (tclrl_last_line) {
        ...
        write_history(default_history_file.c_str());
        const char* result = Tcl_GetStringResult(interp);
        if (strlen(result) > 0)
          printf("%s\n", result);
      }
