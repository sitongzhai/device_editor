...
namespace eda {
  ...
  bool App::canonicalFilePath(std::string & file_path, std::string &canonical_path) {
    ...
    if (strlen(file_path.data()) > MAX_PATH || PathCanonicalizeW(lpBuffer, wc_path) == FALSE) {
     eda_error("*ERROR*: Get real-path failed for path '%s', check the file path length and format.\n", file_path.data());
      return false;
    }
    if (PathFileExists(lpBuffer) == FALSE) {
      eda_error("*ERROR*: File '%s' not exist.\n", file_path.data());
      return false;
    }
    // wchar_t => std::string
    ...
    if (_fullpath(full_path, canonical_path.data(), MAX_PATH) == NULL) {
      eda_error("*ERROR*: Get full-path failed for path '%s'.\n", canonical_path.data());
      return false;
    }
    ...
    if (strlen(path) > MAX_PATH || FALSE == PathCanonicalize(l_path, path)) {
      eda_error("*ERROR*: Get real-path failed for path '%s'.\n", file_path.data());
      return false;
    }
    char full_path[MAX_PATH];
  ...
  bool App::avoidOverwrite(const std::string & file_name) {
    ...
    if ((pDir = opendir(dir_path.c_str())) != NULL) {
      ...
      eda_error("*ERROR*: The directory '%s' does not exist.\n", dir_path.c_str());
    }
#else
    ...
    for (size_t i = 0; i < all_files.size(); i++) {
      ...
      if (found == std::string::npos) {
        eda_warning("*WARNING*: Found a file without suffix: '%s'.\n");
        continue;
      }
    ...
    if (max >= 0) {
      ...
      eda_info("*INFO*: The existing Verilog file '%s' was renamed to '%s' to avoid overwriting.\n", file_name.c_str(), new_file_name.c_str());
    }
    return true;
  }
}
